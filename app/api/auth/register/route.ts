...
export async function POST(req: Request) {
  try {
    const { name, email, password, role, id } = await req.json(); // id রিসিভ করা হচ্ছে
    await dbConnect();

    // ইমেইল বা আইডি দিয়ে চেক
    const existingUser = await User.findOne({ $or: [{ email }, { id }] });
    if (existingUser) {
      return NextResponse.json({ message: "এই ইমেইল বা আইডি দিয়ে আগেই অ্যাকাউন্ট খোলা হয়েছে" }, { status: 400 });
    }
