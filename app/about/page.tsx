    ...
    <div className="space-y-20 pb-20">
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
          <motion.div initial={{ opacity: 0, x: -30 }} whileInView={{ opacity: 1, x: 0 }} className="bg-white p-10 rounded-3xl card-shadow border border-accent/50 space-y-6">
            <div className="w-14 h-14 bg-primary/10 rounded-2xl flex items-center justify-center text-primary"><Target className="w-8 h-8" /></div>
            <h2 className="text-3xl font-bold text-primary">{t('about.missionTitle')}</h2>
            <p className="text-ink/70 leading-relaxed">{t('about.missionDesc')}</p>
          </motion.div>
          <motion.div initial={{ opacity: 0, x: 30 }} whileInView={{ opacity: 1, x: 0 }} className="bg-white p-10 rounded-3xl card-shadow border border-accent/50 space-y-6">
            <div className="w-14 h-14 bg-secondary/10 rounded-2xl flex items-center justify-center text-secondary"><Eye className="w-8 h-8" /></div>
            <h2 className="text-3xl font-bold text-primary">{t('about.visionTitle')}</h2>
            <p className="text-ink/70 leading-relaxed">{t('about.visionDesc')}</p>
          </motion.div>
        </div>
      </section>
        ...
        <div className="flex flex-col md:flex-row gap-16 items-center">
          <div className="flex-1 space-y-8">
            <div className="inline-flex items-center space-x-2 text-primary font-bold">
              <History className="w-6 h-6" /><span>{t('about.historyTitle')}</span>
            </div>
          ...
          <div className="flex-1">
            <img src="https://images.unsplash.com/photo-1541339907198-e08756ebafe3?auto=format&fit=crop&q=80&w=1000" alt="Madrasa History" className="rounded-3xl shadow-2xl" referrerPolicy="no-referrer" />
          </div>
        </div>
      </section>
      ...
      <section className="bg-primary/5 py-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {(t('about.boardMembers') as any[]).map((member: any, index: number) => (
              <motion.div key={index} initial={{ opacity: 0, scale: 0.9 }} whileInView={{ opacity: 1, scale: 1 }} transition={{ delay: index * 0.1 }}
                ...
                <img
                  ...
                  alt={member.name} className="w-32 h-32 mx-auto rounded-full object-cover border-4 border-primary/10" referrerPolicy="no-referrer"
                />
                <div>
