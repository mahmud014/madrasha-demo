    ...
    return (
      <div className="bg-white rounded-2xl p-6 card-shadow border border-accent/50 animate-pulse">
        ...
        <div className="space-y-4">
          {[1, 2, 3].map(i => <div key={i} className="h-12 bg-accent rounded"></div>)}
        </div>
      </div>
    );
  }
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center space-x-3">
          <div className="bg-secondary/10 p-2 rounded-lg">
            <Bell className="w-6 h-6 text-secondary animate-bounce" />
          </div>
          ...
          notices.map((notice, index) => (
            ...
            >
              ...
              <div className="flex-1">
                <h4 className="text-sm font-medium text-ink group-hover:text-primary transition-colors line-clamp-2">
                  {notice.title}
                </h4>
              </div>
            </motion.div>
