        ...
        <div className="bg-white p-8 rounded-3xl card-shadow border border-accent/50 space-y-8">
          <form onSubmit={handleSearch} className="flex flex-col md:flex-row gap-4">
            ...
            <button
              type="submit"
              disabled={loading}
              className="bg-secondary hover:bg-secondary/90 text-white font-bold px-10 py-4 rounded-2xl transition-all disabled:opacity-50"
            >
          ...
          {result && result !== 'not_found' && (
            <motion.div
              ...
              className="border-t border-accent pt-8 space-y-6"
            >
              <div className="flex items-center justify-between">
                ...
                <div className="text-right">
                  ...
                  <div className="flex items-center text-green-600 font-bold text-sm">
                    <CheckCircle className="w-4 h-4 mr-1" /> {result.exam || t('results.finalExam')}
                  </div>
                </div>
              </div>
            </motion.div>
