        ...
        <div className="flex justify-between h-20">
          <div className="flex items-center">
            <Link href="/" className="flex items-center space-x-3">
              <div className="w-12 h-12 bg-primary rounded-full flex items-center justify-center text-white font-bold text-xl">ম</div>
          ...
          <div className="hidden lg:flex items-center space-x-0.5">
            {navItems.map((item) => (
              <Link
                ...
                className={cn(
                  ...
                  pathname === item.path ? "text-primary bg-primary/5" : "text-ink/70 hover:text-primary hover:bg-primary/5"
                )}
              >
            ...
            <div className="flex items-center space-x-1.5 ml-2 border-l border-accent/30 pl-2">
              <button onClick={toggleLanguage} className="flex items-center space-x-1 px-2 py-1.5 rounded-lg bg-accent/30 text-ink hover:bg-accent/50 transition-all text-[11px] font-bold">
              ...
              >
                {isAdmin ? <><Lock className="w-3.5 h-3.5" /><span>{t('nav.dashboard')}</span></> : <><User className="w-3.5 h-3.5" /><span>{t('nav.login')}</span></>}
              </Link>
            </div>
          </div>

          <div className="lg:hidden flex items-center space-x-4">
            <button onClick={toggleLanguage} className="flex items-center space-x-1 px-3 py-1.5 rounded-lg bg-accent/30 text-ink hover:bg-accent/50 transition-all text-xs font-bold">
              <Globe className="w-3.5 h-3.5" />
              <span>{language === 'bn' ? 'EN' : 'বাংলা'}</span>
            </button>
            <button onClick={() => setIsOpen(!isOpen)} className="text-ink hover:text-primary p-2">
            ...
            <div className="px-2 pt-2 pb-3 space-y-1">
              {navItems.map((item) => (
                <Link
                  ...
                  className={cn(
                    "flex px-3 py-3 rounded-md text-base font-medium items-center space-x-3",
                    pathname === item.path ? "text-primary bg-primary/5" : "text-ink/70 hover:text-primary hover:bg-primary/5"
                  )}
                >
              ...
              <Link href="/admin" onClick={() => setIsOpen(false)} className="flex px-3 py-4 rounded-md text-base font-bold text-primary bg-primary/5 items-center space-x-3">
